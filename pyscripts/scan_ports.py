...
def get_arguments(args):
    parser = argparse.ArgumentParser(
        description="Scan ports for a given host in a given range"
    )
    parser.add_argument(
        "-t", "--target", help="The address of the machine to scan", required=True
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    parser.add_argument(
        "--timeout",
        help="Set the timeout per request in seconds",
        default=0.3,
        type=float,
    )
    ...
    port_group.add_argument(
        "--all", help="Scan ports range 0-65535", action="store_true"
    )
